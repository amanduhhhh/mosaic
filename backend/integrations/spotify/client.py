    ...
    def fetch_user_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            top_songs = [{
                ...
                "album": t['album']['name'],
                "plays": t['popularity'],  # Use popularity as proxy for plays
            ...
            top_genres = [g.title() for g, _ in sorted(genre_counts.items(), key=lambda x: x[1], reverse=True)[:5]]
            
            top_artists = [{
                "name": a['name'],
                "plays": a['popularity'],  # Use popularity as proxy for plays
                "followers": a.get('followers', {}).get('total', 0)
            } for a in artists['items'][:10]]
            
            # Calculate total minutes from recent plays
            total_duration_ms = sum(item['track']['duration_ms'] for item in recent['items'])
            ...
            return {
                ...
                "listening_history": [
                    {"month": "Recent", "minutes": total_minutes}
                ],
